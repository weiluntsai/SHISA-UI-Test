...
function MainLayout() {
  ...
  const [currentView, setCurrentView] = useState<'live' | 'playback' | 'downloads' | 'users' | 'configuration'>('live');
  ...
  const handleSidebarClick = (view: 'live' | 'playback' | 'downloads' | 'users' | 'configuration') => {
    setCurrentView(view);
    if (window.innerWidth < 768) setIsSidebarVisible(false);
  };

  return (
    <div className="flex h-screen w-full bg-gray-100 dark:bg-black text-slate-800 dark:text-slate-100 font-sans overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 py-4 overflow-y-auto overflow-x-hidden">
          <div className="px-6 mb-2 text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">{t.monitor}</div>
          ...
          <SidebarItem 
            ...
            collapsed={false}
            onClick={() => handleSidebarClick('playback')}
          />
          <SidebarItem 
            icon={Download} 
            label={t.downloads} 
          ...
          <div className="px-6 mt-6 mb-2 text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">{t.system}</div>
          <SidebarItem 
            icon={Users} 
            label={t.users} 
          ...
          <SidebarItem 
            ...
            active={currentView === 'configuration'}
            collapsed={false} 
            onClick={() => handleSidebarClick('configuration')}
          />
        </div>
      </div>
