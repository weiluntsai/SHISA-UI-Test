...
const Timeline: React.FC<TimelineProps> = ({ hideControls = false }) => {
  const { t } = useLanguage();
  const hours = Array.from({ length: 24 }, (_, i) => i);
  
  return (
    <div className={`flex flex-col select-none shrink-0 bg-[#121212] border-t border-gray-800 ${hideControls ? 'h-16' : 'h-24'}`}>
      {!hideControls && (
        <div className="h-8 flex items-center justify-between px-4 border-b border-gray-800 bg-[#1a1a1a]">
           <div className="flex items-center gap-4">
              ...
              <div className="text-xs text-blue-400 font-mono cursor-pointer hover:text-blue-300 hidden sm:block">1X {t.speed}</div>
           </div>
           <div className="flex gap-2">
